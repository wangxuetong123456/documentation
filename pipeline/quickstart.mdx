...
```python
from xparse_pipeline import LocalSource
...
```python
from xparse_pipeline import FtpSource
...
```python
from xparse_pipeline import MilvusDestination
...
```python
from xparse_pipeline import LocalDestination
...
pipeline = Pipeline(
    ...
    parse_config=parse_config,
    chunk_config=chunk_config_by_page,
    embed_config=embed_config_v4
)
...
```python
from xparse_pipeline import LocalSource, LocalDestination, Pipeline, ParseConfig, ChunkConfig, EmbedConfig
...
- **阅读[Agent/RAG 实战教程](/pipeline/tutorial/overview)**：了解如何使用 xParse ETL 构建完整的 RAG 应用与 LangChain Agent
