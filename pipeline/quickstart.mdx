---
title: "快速启动"
description: "5 分钟快速上手 xParse Pipeline，从安装到运行第一个文档处理流程"
---

<Tip>
  本教程基于 Python 示例分步讲解如何使用 xParse Pipeline。我们提供了完整的示例代码，可在本地一键运行，助您快速体验 Pipeline 的强大能力。
</Tip>

## 为什么使用 xParse Pipeline？

在大模型和 RAG 应用开发中，文档处理是一个关键环节。传统方式需要分别调用解析、分块、向量化等多个接口，流程复杂且容易出错。xParse Pipeline 通过统一的 Pipeline API，一次性完成文档解析、智能分块和向量化全流程，让您专注于业务逻辑，而不是繁琐的数据处理。

使用 xParse Pipeline，您可以：
...
如果您正在构建 RAG 应用、知识库系统或 Agent 应用，xParse Pipeline 会是您的得力助手。

## 如何使用 xParse Pipeline？

您可以参考以下示例和步骤，快速验证并将 xParse Pipeline 接入到您的系统中。

### 先决条件：获取 API Key

使用 xParse Pipeline 处理文档时，您需要先获取 API Key。请先登录后前往 [TextIn 工作台 - 账号与开发者信息](https://www.textin.com/console/dashboard/setting) 获取您的 `x-ti-app-id` 和 `x-ti-secret-code`。
...
1. **通过配置创建**（推荐）：使用 JSON 配置，简单直观，易于管理（可以是独立文件）
2. **手动创建组件**：手动创建配置对象传入Pipline，更灵活
...
- **阅读[RAG 场景教程](/pipeline/rag-tutorial)**：了解如何构建完整的 RAG 应用
- **查看[API 文档](/api-reference/endpoint/pipeline)**：了解 Pipeline API 的详细参数和返回格式
- **探索最佳实践**：学习分块策略选择、性能优化等技巧
