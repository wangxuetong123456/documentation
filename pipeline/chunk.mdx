...
参考 [Unstructured 官方文档](https://docs.unstructured.io/ui/chunking)，xParse Pipeline 在 chunk 阶段同样只输出以下几类元素：
...
- 高级模式：若按 Unstructured 的 gzip+Base64 方案配置，`orig_elements` 会是一串压缩后的 JSON，需按"Base64 → gzip → UTF-8"顺序解码才能获取原文（用于对齐 Unstructured 生态或保留图片/表格的原始元信息）。
