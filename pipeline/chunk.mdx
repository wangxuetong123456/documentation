---
title: "文本分块 - Chunk"
description: "了解 xParse ETL 中的文本分块模块，将文档元素组合成适合向量化的块"
---
...
xParse ETL 支持三种分块策略，每种策略适用于不同的场景：
...
xParse ETL 在 chunk 阶段同样只输出以下几类元素：
...
- 高级模式：若按 gzip+Base64 方案配置，`orig_elements` 会是一串压缩后的 JSON，需按"Base64 → gzip → UTF-8"顺序解码才能获取原文（用于保留图片/表格的原始元信息）。
...
```python
from xparse_pipeline import ChunkConfig
