...
文档解析是将非结构化文件内容提取并转换为结构化[文档元素](/pipeline/elements-metadata)的过程。xParse ETL 的 parse 阶段封装了 [xparse 文档解析 API](/xparse/parse-quickstart)，参数和功能保持一致。

与简单将文件视为纯文本不同，xParse ETL 通过解析保留了文件的语义结构，让您能够更灵活地控制后续处理，并充分考虑文件的结构特点。同时，通过将不同文件格式统一转换为 xParse ETL 的元素类型，无论源文件是什么格式，您都可以在下游处理中采用相同的方式。
