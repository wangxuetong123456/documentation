---
title: "数据源 - Sources"
description: "了解 xParse ETL 支持的数据源连接器，包括 S3 兼容服务、本地文件夹和 FTP 远程文件服务"
---

## 概念

数据源连接器（Sources）用于从不同存储系统中读取文档文件。xParse ETL 支持多种数据源类型，包括 S3 兼容的对象存储服务、本地文件系统和 FTP 远程文件服务。
