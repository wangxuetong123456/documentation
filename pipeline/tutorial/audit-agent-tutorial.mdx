...
export MILVUS_URI=./audit_vectors.db
...
AUDIT_PIPELINE_CONFIG = {
    "source": {
        ...
        "pattern": ["*.pdf", "*.xlsx", "*.xls", "*.png", "*.jpg"]  # 支持多种财务文档格式
    },
    "destination": {
        "type": "milvus",
...
def run_audit_pipeline(job_name: str = None) -> dict:
    """运行审计文档处理Pipeline"""
    pipeline = create_pipeline_from_config(AUDIT_PIPELINE_CONFIG)
    stats = pipeline.run(job_name=job_name or f"audit-job-{int(time.time())}")
    return {
        "job_name": job_name,
...
vector_store = Milvus(
    ...
    collection_name="audit_documents",
    connection_args={"uri": os.getenv("MILVUS_URI", "./audit_vectors.db")}
)
...
def detect_anomalies(query: str) -> str:
    ...
    # 1. 金额异常
    if all_amounts:
        amounts_values = [a["value"] for a in all_amounts]
        max_amount = max(amounts_values)
        min_amount = min(amounts_values)
        
        if max_amount / min_amount > 1000:  # 金额差异过大
            anomalies.append({
                "type": "金额差异异常",
                "description": f"最大金额 {max_amount} 与最小金额 {min_amount} 差异过大",
                "details": [a for a in all_amounts if a["value"] in [max_amount, min_amount]]
            })
...
agent = initialize_agent(
    ...
    agent_kwargs={
        "prefix": """你是一个专业的财务审计助手。你的任务是帮助审计人员：
1. 从财务文档中提取关键信息
2. 检查文档的合规性
3. 检测数据异常
4. 检索历史审计案例
        ...
        for doc in docs:
            ...
            for amount_str in amounts:
                try:
                    ...
                    pass
        if all_amounts and max(all_amounts) / min(all_amounts) > 1000:
            anomalies.append("金额差异异常")
...
**Q: 如何提高提取准确率？**  
A: 1) 使用 `text-embedding-v4` 高精度模型；2) 优化分块策略；3) 增加检索的文档数量（k值）。
