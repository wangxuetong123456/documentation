...
- **Agent 行为**：当客服输入“有客户反馈 3.2.1 版本连接失败怎么处理？”时，Agent 先调用 `vector_search`；若检索不到最新版本，则触发 `run_xparse_pipeline` 解析 `faqs/3.2.1/` 目录，之后再次检索并返回引用。
...
- **Agent 行为**：学习者提问“请解释热力学第二定律并给出实验例子”，Agent 检索目标课程章节，再根据 `language` 字段决定回复语言或是否需要调用 `run_xparse_pipeline` 同步最新课件。
