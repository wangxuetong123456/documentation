...
下面是一个完整的、可以直接运行的示例，其中的 `X_TI_APP_ID` 与 `X_TI_SECRET_CODE` 请至 [TextIn官方网站](https://www.textin.com) 获取，示例中使用 `通义千问` 的大模型能力：
...
def chat_with_agent(query: str, session_id: str = "default"):
    ...
    # 保存对话历史
    chat_history[session_id].append((query, response["output"]))
