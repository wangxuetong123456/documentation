...
3. 切换到**智能抽取**tab，并切换到**字段模式**；
   ...
   ![Extract V3 Schema Empty Jp](https://static.textin.com/docs/images/extract-v3-schema-empty.jpg)
...
9. 重复以上步骤，直到添加完您想抽取的全部字段；
   ...
   <img
     src="https://static.textin.com/docs/images/extract-v3-schema-edit.png"
...
1. 抽取完成后，会展示抽取结果面板，您可以点击字段抽取结果，会在左侧原文件区域找到对应的坐标边界（高亮显示）；
   ...
   ![Extract V3 Schema Result Panel Jp](https://static.textin.com/docs/images/extract-v3-schema-result-panel.jpg)
2. 抽取结果默认显示**预览**面板，您也可以切换到**JSON**面板查看对应的结构化数据；

   <img
     src="https://static.textin.com/docs/images/extract-v3-json-preview.jpg"
...
![Extract V3 Mono Key Jp](https://static.textin.com/docs/images/extract-v3-mono-key.jpg)

### 抽取多值

当待抽取的字段存在多个并列的值时，推荐多值抽取，使用`array/string`，`array/number`等类型。

![Extract V3 Multi Value Jp](https://static.textin.com/docs/images/extract-v3-multi-value.jpg)

![Extract V3 Multi Value Demo Jp](https://static.textin.com/docs/images/extract-v3-multi-value-demo.jpg)

### 抽取表格

当待抽取的内容有多组重复属性的数据组成，一般在原文档中呈现为类表格形式时，推荐表格抽取，使用`array/object`加单值字段嵌套表示。

![Extract V3 Schema Table Sample Jp](https://static.textin.com/docs/images/extract-v3-schema-table-sample.jpg)

如果文件中存在多个表格，可以添加多个`array/object`类型的字段。

![Extract V3 Schema Multi Table Jp](https://static.textin.com/docs/images/extract-v3-schema-multi-table.jpg)

### 抽取嵌套对象

当待抽取的内容由多个相关联的属性字段组成，推荐使用嵌套对象抽取，使用`object`类型。

![Extract V3 Object Jp](https://static.textin.com/docs/images/extract-v3-object.jpg)