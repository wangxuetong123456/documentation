...
在实际使用过程中，您可能会需要在一定时间内集中性的批量解析文档；在这种情况下，即使TextIn xParse 文档解析API本身的速度足够快，但依次逐个解析大批量文档所需要的总耗时也可能会比较长。

针对这种情况，TextIn xParse 文档解析API支持多并发请求，默认2 QPS，最大可支持10 QPS。帮助您快速高效的完成大批量文档解析工作。
