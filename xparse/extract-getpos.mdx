...
参考[**快速启动**](/xparse/extract-quickstart)中的返回结果示例，文档抽取API的返回结果中包含了精确的坐标信息，您可以参考以下教程获取坐标并完成前端可视化工作。

### 数据结构分析

如[**快速启动**](/xparse/extract-quickstart)中的返回结果示例(prompt模式返回示例节选)，抽取结果中包含以下关键信息：
...
![Annotated Extract Result Pn](/images/annotated_extract_result.png)
