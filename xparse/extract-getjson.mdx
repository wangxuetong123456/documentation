...
- **page_count**：智能文档抽取处理的文档页数，超过最大页数限制时（100页），返回为最大页数。
...
### raw_json：大模型抽取带坐标信息的抽取结果

仅当传入prompt参数时返回，包含详细的位置信息和边界框数据，用于高级处理场景。
...
- **durations**：当前页总耗时(毫秒)
- **image_id**：当前页图片id 。受URL参数影响，_\_<u>当URL参数image_output_type=default且get_image=page/both时返回，</u>\__详情见[快速启动-URL参数说明](/xparse/extract-quickstart)。
...
- **origin_image_id**：切边或去水印前的原始页图片。受URL参数切边矫正或去水印影响，详情见[快速启动-URL参数说明](/xparse/extract-quickstart)。仅当开启切边或去水印，_<u>image_output_type=default且get_image=page/both时返回。</u>_
  - 下载方式同image_id
- **width**：文档页宽度
- **height**：文档页高度
- **angle**：图像（中文字）的角度（当输入为图像时，默认为0， 可选值0, 90, 180, 270)

### usage：大模型抽取的token消耗情况

仅当传入prompt参数时返回。

- **prompt_tokens**：大模型抽取消耗输入token数量。
- **completion_tokens**：大模型抽取消耗输出token数量。
- **total_tokens**：大模型抽取消耗token总量。
