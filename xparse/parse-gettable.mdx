...
您可以将解析获取到的表格元素保存为 md 和 json 以及 excel 格式的文件。

### 将表格保存为 md 和 json 文件

- 
- 参考[快速启动](/xparse/parse-quickstart)，在 options 中设置参数 table_flavor 为 md 或 html，这样API会以Markdown或HTML格式输出表格。
...
- 参考[快速启动](/xparse/parse-quickstart)，在 options 中设置参数 get_excel=1，让API返回 excel_base64 字段（Excel文件的base64编码）。
