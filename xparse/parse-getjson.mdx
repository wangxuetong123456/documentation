...
- **detail**：markdown详细信息，受URL参数**markdown_details**影响，默认生成markdown details。详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  ...
  - **type**：类型。
    ...
    - image（图片类型）
    - table（表格类型）
  - **image_url**：受URL参数**get_image**和**image_output_type**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  ...
  - **origin_position**：受URL参数**切边矫正**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。仅当打开切边时返回，表示该段落在原图中的坐标。格式同position。
...
- **pages**：文档按页为单位展开时， 存储每一页的详情和状态（适用于PDF），部分信息与**metrics**字段重复。受URL参数**page_details**影响，默认返回pages，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  ...
  - **raw_ocr**：全文识别结果，只包含文字结果。受URL参数**page_details**和**raw_ocr**影响，默认不返回，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
