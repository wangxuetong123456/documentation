...
### detail：markdown 详细信息，按type组织

detail表示markdown中的不同类型

- **detail**：markdown详细信息，受URL参数**markdown_details**影响，默认生成markdown details。详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  ...
  - **origin_position**：image类型。受URL参数**切边矫正**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。仅当打开切边时返回，表示该段落在原图中的坐标。格式同**position**。
  - **content**：表示段落的类型。0 正文(段落、图片、表格)；1 非正文(页眉、页脚、侧边栏)
  - **type**：类型。
  ...
  - **image_url**：image类型独有。受URL参数**get_image**和**image_output_type**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  ...
  - **tags**：paragraph类型独有。表示段落内是否存在特殊文本，类型包括公式formula和手写体handwritten。
  - **caption_id**：image和table类型才有。表格或图片的标题id。
    - **page_id**：标题所在页码。
  ...
  - **cells**：单元格数组, 仅在**type**为table时返回。
    ...
    - **origin_position**：受URL参数**切边矫正**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。仅当打开切边时返回，表示该单元格在原图中的坐标。格式同position。
    - **text**：单元格文本内容。
    - **type**：类型，固定为cell，表示单元格。

### pages：每一页的详情和状态

- **pages**：文档按页为单位展开时， 存储每一页的详情和状态（适用于PDF），部分信息与**metrics**字段重复。受URL参数**page_details**影响，默认返回pages，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
  - **page_id**：当前页码 (若为流式文件, 页码置为0)，例如"0"。
  - **status**：表示当前页的引擎输出状态，或者error_message，例如"success"。
  - **durations**：当前页总耗时(毫秒)，例如"612.5"。
  - **width**：文档页宽度
  - **height**：文档页高度
  - **angle**：图像的角度（当输入为图像时，默认为0， 可选值0, 90, 180, 270)
  - **image_id**：当前页图片id 。受URL参数**get_image**和**image_output_type**影响，当URL参数image_output_type=default且get_image=page/both时返回，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
    - 下载方式：**https://api.textin.com/ocr_image/download?image_id=xxx** ,需要在headers里添加appid和key。例如使用curl下载：
...
- **catalog**：目录树结构。受URL参数**catalog_details**影响，详情见[快速启动-URL参数说明](/xparse/parse-quickstart)。
...
部分信息跟**pages**字段重复，当URL参数**page_details**设置为不返回**pages**字段时，可以在**metrics**字段获取每一页的信息。
