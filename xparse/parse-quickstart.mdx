...
- **remove_watermark**（**去水印**）：是否进行去水印处理，默认为0，不去水印。
  - 0 不去水印
  - 1 去水印
- （**图表识别**）：是否开启图表识别，开启图表识别会将识别到的图表以表格形式输出。默认为0，不进行图表识别。
  - 0 不开启图表识别
  - 1 开启图表识别
