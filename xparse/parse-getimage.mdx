...
Textin xParse为保护您的数据隐私，从文档解析API返回的图片链接有效期为30天，30天后平台会自动删除图片资源。当您想要获取图片到本地并持久化时，有两种选择：

1. 设置[image-output-type](https://docs.textin.com/api-reference/endpoint/parse#parameter-image-output-type)参数为base64str，此时图片直接以base64格式在AP结果中返回（长文档不推荐）。
2. 设置[image-output-type](https://docs.textin.com/api-reference/endpoint/parse#parameter-image-output-type)参数为default（不传时默认为该至），此时图片直接以textin平台链接方式返回，您可通过链接下载图片到本地，或上传到您的云存储。
