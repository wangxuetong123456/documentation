...
- coord_restore：调整各字段的返回坐标逻辑，选择是参照原图还是切图坐标系返回坐标，默认为0，以切图后转正图为基准。
  - 0 坐标以切图后转正图为基准
  - 1 坐标以原图为基准
- is_same_file：当结构化多页pdf时，选择是否将多页pdf文件作为一个整体进行识别，来保证这一套票的信息结构化完整性（如你可能会遇到一份多页的出院小结票据），默认为0，将多页pdf进行拆分后，逐个进行识别。<br/>
...
```json
{
    ...
    "result": {
        ...
        "object_list": [  // 票据识别结果
            {
                "details": {
                    "invoice_information": {                 // 费用汇总信息
                        "document_title_name": {             // key
                            "description": "上海市医疗门诊收", // key中文描述
                            "value": "0",                    // key识别结果
                            "bounding_regions": [            // key相关的坐标信息
                                "value": "上海",             // 一行的识别内容
                                "position": [                // 改行图中的坐标位置
                                    276,
                                    208,
                ...
                "type": "inpatient_medical_invoice"， // 票据类型
                "type_description": "住院发票"         // 票据类型中文描述
            }
        ],
        "page_count": 1,                 // 样本总页数
        "ppi": 144,                      // PDF样本解析PPI
